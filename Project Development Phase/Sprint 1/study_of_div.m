...
for mr = 1:length(Mr)
    ...
    for k = 1:length(K)
        ...
        for m = 1:length(M)
            ...
            % Channel
            h = mu + s*(1/sqrt(2))*(randn(Mr(mr),Mt,snapshots/Mt)+ 1j*randn(Mr(mr),Mt, snapshots/Mt));
            H(:,:,1:2:end-1) = h;
            H(:,:,2:2:end) = h;
            ...
            for c = 1:100
                for jj = 1:length(EbNo)
                    noisy_mod = awgn(ric_msg,snr(jj),'measured','dB'); decodeData = step(ostbcComb,noisy_mod,pathGainself);
...
figure(1) 
semilogy(EbNo,sum_BER(:,1,1,1),'r-o',EbNo,sum_BER(:,2,1,1),'g-o',EbNo,ric_ber(:,1,1,1),'r-',EbNo,ric_ber(:,2,1,1),'g-', EbNo,sum_BER(:,1,1,2),'b-o',EbNo,sum_BER(:,2,1,2),'y-o', EbNo,ric_ber(:,1,1,2),'b-',EbNo,ric_ber(:,2,1,2),'y-','LineWidth', 1.5)
title('Rician model (K = 4.0)')
legend('QPSK(simulated) 2x1', '8-PSK(simulated) 2x1','QPSK(theory) 2x1','8-PSK(theory) 2x1', 'QPSK(simulated) 2x2', '8-PSK(simulated) 2x2','QPSK(theory) 2x2','8-PSK(theory) 2x2')
xlabel('EbNo (dB)')
ylabel('BER')
grid on
figure(2) 
semilogy(EbNo,sum_BER(:,1,2,1),'r-o',EbNo,sum_BER(:,2,2,1),'g-o',EbNo,ric_ber(:,1,2,1),'r-',EbNo,ric_ber(:,2,2,1),'g-', EbNo,sum_BER(:,1,2,2),'b-o',EbNo,sum_BER(:,2,2,2),'y-o', EbNo,ric_ber(:,1,2,2),'b-',EbNo,ric_ber(:,2,2,2),'y-', 'LineWidth', 1.5)
